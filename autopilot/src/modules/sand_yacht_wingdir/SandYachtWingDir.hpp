...
#define SLIDING_WINDOW 3 // number of samples to average for wing direction

class SandYachtWingDir final :
	public ModuleBase<SandYachtWingDir>,
	public px4::ScheduledWorkItem

{
public:
	SandYachtWingDir(void);
	~SandYachtWingDir(void);
	SandYachtWingDir(const SandYachtWingDir &) = delete;
	SandYachtWingDir &operator=(const SandYachtWingDir &other) = delete;
...
// uorb subscriptions
	uORB::Subscription _wind_direction_sub{ORB_ID(wing_direction)};
	uORB::Publication<sand_yacht_wing_dir_s> _wind_direction_pub{ORB_ID(sand_yacht_wing_dir)};

	wing_direction_s wind_direction = {0};
	sand_yacht_wing_dir_s sand_yacht_wing_direction = {0};
